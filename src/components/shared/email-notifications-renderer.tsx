...
import { X, Mail, AlertTriangle, Clock, ArrowRight } from "lucide-react";
      ...
      {activeEmails.map((email) => {
        ...
        return (
          ...
          >
            ...
            {/* Lista de Alertas Críticos no Email */}
            <div className="p-4 space-y-3 max-h-[320px] overflow-y-auto">
              <div className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                Conteúdo do Alerta ({email.alerts.length} item{email.alerts.length > 1 ? "s" : ""}):
              </div>

              <div className="space-y-2.5">
                {email.alerts.map((alert, idx) => (
                  <div
                    key={idx}
