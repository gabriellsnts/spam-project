...
export default function Home() {
  ...
  const getIcon = (type: DomainType) => {
    ...
    switch (type) {
      case "maintenance":
        return <Wrench className={`${size} text-red-500`} />;
      case "demand":
  ...
  const getBorderColor = (type: DomainType, isActive: boolean) => {
    if (isActive) {
      switch (type) {
        case "maintenance":
          return "border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.2)] bg-red-500/[0.02] dark:bg-red-950/[0.08]";
        case "demand":
        ...
        case "credit-risk":
          return "border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.2)] bg-blue-500/[0.02] dark:bg-blue-950/[0.08]";
      }
    }
    
    switch (type) {
      case "maintenance": return "hover:border-red-500/50 hover:shadow-[0_0_25px_rgba(239,68,68,0.12)]";
      case "demand": return "hover:border-emerald-500/50 hover:shadow-[0_0_25px_rgba(16,185,129,0.12)]";
      case "churn": return "hover:border-violet-500/50 hover:shadow-[0_0_25px_rgba(139,92,246,0.12)]";
      case "credit-risk": return "hover:border-blue-500/50 hover:shadow-[0_0_25px_rgba(59,130,246,0.12)]";
    }
  };

  const getGlowBg = (type: DomainType) => {
    switch (type) {
      case "maintenance": return "from-red-500/10";
      case "demand": return "from-emerald-500/10";
      case "churn": return "from-violet-500/10";
      case "credit-risk": return "from-blue-500/10";
    }
  };

  return (
    <div className="flex flex-col items-center justify-center py-4 md:py-8 max-w-5xl mx-auto w-full relative z-10 animate-in fade-in duration-500">
      {/* Hero Section */}
      <div className="text-center max-w-2xl mb-12 space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <h1 className="text-4xl md:text-5xl font-black tracking-tight leading-tight text-transparent bg-clip-text bg-gradient-to-b from-foreground to-foreground/75">
          ...
          return (
            ...
            >
                ...
                {isActive && (
                  <span className="flex h-2.5 w-2.5 relative">
                    <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${
                      domain.id === "maintenance" ? "bg-red-400" : domain.id === "demand" ? "bg-emerald-400" : domain.id === "churn" ? "bg-violet-400" : "bg-blue-400"
                    }`} />
                    <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${
                      domain.id === "maintenance" ? "bg-red-500" : domain.id === "demand" ? "bg-emerald-500" : domain.id === "churn" ? "bg-violet-500" : "bg-blue-500"
                    }`} />
                  </span>
                )}
              </CardHeader>
              ...
              <CardContent className="space-y-4 relative z-10">
                <CardDescription className="text-muted-foreground text-xs leading-relaxed min-h-[48px]">
                  {domain.description}
                </CardDescription>
      ...
      {/* Footer Info */}
      <div className="text-muted-foreground/40 text-[10px] text-center mt-16 font-mono border-t border-border/80 pt-6 w-full max-w-md">
